...
class ScaledQuantDescriptor():
    ...
    def __init__(self, num_bits=8, name=None, **kwargs):
        ...
        self._learn_scale = False if self.quantizer_type == 'naive' else True
...
def _tensor_quant_scale_offset(inputs, scale, offset, max_bound=127, min_bound=-128):
    ...
    scale = scale + epsilon
    outputs = torch.clamp(((inputs - offset) / scale).round_(), min_bound, max_bound)
    # restore scale value
    scale = scale - epsilon
    ...
    return (outputs * scale + offset).to(inputs.dtype)
