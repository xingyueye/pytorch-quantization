...
from mtpq.tensor_quant import QuantDescriptor, tensor_quant, fake_tensor_quant
...
class QuantParamTensor(torch.Tensor):
    @staticmethod
    def __new__(cls, a):
        shape = a.shape
        kwargs = {}
    ...
    def __init__(self, tensor_to_wrap):
        super(QuantParamTensor, self).__init__()
        self._tensor = tensor_to_wrap
    ...
    def __torch_dispatch__(cls, func, types, args=(), kwargs=None):
        ...
        def unwrap(e):
            return e._tensor if isinstance(e, QuantParamTensor) else e

        # Wrap back all Tensors into our custom class
        def wrap(e):
            # Check for zeros and use that to get indices
            return QuantParamTensor(e) if isinstance(e, torch.Tensor) else e

        rs = tree_map(wrap, func(*tree_map(unwrap, args), **tree_map(unwrap, kwargs or {})))
        # print(f'QuantParamTensor __torch_dispatch__ func: {func}, return :{rs}')
        return rs

    def __repr__(self):
        return super().__repr__(tensor_contents=f"values={self._tensor}")

    def half(self):
        print(f'QuantParamTensor half, will do nothing')
        return self
...
class TensorQuantizer(nn.Module):
    ...
    def __init__(self, quant_desc=QuantDescriptor(), disabled=False, if_quant=True, if_clip=False, if_calib=False):
        ...
        if quant_desc.amax is not None:
            ...
            self.register_buffer('_amax', QuantParamTensor(torch.tensor(quant_desc.amax)))
            print(f'_amax inside __init__, registered')
    ...
    def amax(self, value):
        if value is None:
            ...
            if not hasattr(self, "_amax"):
                self.register_buffer('_amax', QuantParamTensor(torch.tensor(value)))
                print(f'_amax inside amax @setter, registered {value}')
            else:
                value = torch.tensor(value, device=self._amax.device, dtype=self._amax.dtype)
                ...
                print(f'_amax inside amax @setter, copyed {value}')
    ...
    def load_calib_amax(self, *args, **kwargs):
        ...
        *args and **kwargs are directly passed to compute_amax, except "strict" in kwargs. Refer to
        ...
        if not hasattr(self, '_amax'):
            ...
            self.register_buffer('_amax', QuantParamTensor(calib_amax.data))
            print(f'_amax inside load_calib_amax, copyed {calib_amax}')
        else:
    ...
    def _get_amax(self, inputs):
        ...
        # print(f"_get_amax, hasattr: {hasattr(self, '_amax')}, _amax: {self._amax if hasattr(self, '_amax') else None}")
        return amax._tensor if isinstance(amax, QuantParamTensor) else amax
    ...
    def _quant_forward(self, inputs):
        ...
        print(f'inside qutizer quant forward, show dtype, inputs: {inputs.dtype}, amax: {amax.dtype}, {amax}, outputs: {outputs.dtype}')
        return outputs
    ...
    def _load_from_state_dict(self, state_dict, prefix, *args, **kwargs):
        ...
        print(f'_load_from_state_dict, dst_has_amax: {dst_has_amax}')
        if not src_has_amax and dst_has_amax:
            ...
            self.register_buffer("_amax", QuantParamTensor(state_dict[prefix + '_amax'].data.cuda()))
            print(f"_amax inside _load_from_state_dict, register {state_dict[prefix + '_amax'].data.cuda()}, {QuantParamTensor(state_dict[prefix + '_amax'].data.cuda())}, now hasattr: {hasattr(self, '_amax')}: {self._amax if hasattr(self, '_amax') else None}")
        ...
        print(f"_amax after super  _load_from_state_dict, now hasattr: {hasattr(self, '_amax')}: {self._amax if hasattr(self, '_amax') else None}")
