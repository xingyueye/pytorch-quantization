...
def reduce_min(input, axis=None, keepdims=True):
    """Compute the absolute maximum value of a tensor.
    ...
    with torch.no_grad():
        output = input
        if axis is None:
            output = torch.min(input)
        else:
            # NotImplementedError, "Support to activation only, Channel-Wise Minmax Calibrator need to be complete!"
            if isinstance(axis, int):
                output, _ = torch.min(output, dim=axis, keepdim=keepdims)
            else:
                if isinstance(axis, tuple) and len(axis) > input.dim():
                    raise ValueError("Cannot reduce more axes than tensor's dim.")
                for i in axis:
                    output, _ = torch.min(output, dim=i, keepdim=True)
                if not keepdims or output.numel() == 1:
                    output.squeeze_()
        return output
def reduce_max(input, axis=None, keepdims=True):
    """Compute the absolute maximum value of a tensor.
...
def reduce_abs_mean(input, axis=None, keepdims=True):
    ...
    with torch.no_grad():
        ...
        if axis is None:
            ...
            if isinstance(axis, int):
                output = torch.mean(output, dim=axis, keepdim=keepdims)
            else:
                if isinstance(axis, tuple) and len(axis) > input.dim():
                    raise ValueError("Cannot reduce more axes than tensor's dim.")
                for i in axis:
                    output = torch.mean(output, dim=i, keepdim=True)
                if not keepdims or output.numel() == 1:
                    output.squeeze_()
        return output