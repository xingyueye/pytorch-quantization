...
parser.add_argument('--calib_file_path', type=str, default=None)
parser.add_argument('--save_qdq_onnx', action='store_true',
                    help='use pre-trained model')
parser.add_argument('--drop_layers', type=int, default=0,
...
def skip_quant_layers(sensitivity_list, model, skip_layers=0):
    model_quant_enable(model)
    count = 0
    for layer_name, sensitivity in sensitivity_list:
        module_quant_disable(model, layer_name)
        count += 1
        if count == skip_layers:
            return
...
def main(args):
    ...
    calib_weight = os.path.join(args.calib_file_path, args.model + '_calib.pth')
    if not os.path.exists(calib_weight):
        with torch.no_grad():
            collect_stats(model, train_loader, args.calib_num)
            compute_amax(model, method=args.method, percentile=args.percentile)
        torch.save(model.state_dict(), os.path.join(args.calib_file_path, args.model + '_calib.pth'))
    else:
        print("|| Load calibrated checkpoint from {}".format(calib_weight))
        model.load_state_dict(torch.load(calib_weight))
